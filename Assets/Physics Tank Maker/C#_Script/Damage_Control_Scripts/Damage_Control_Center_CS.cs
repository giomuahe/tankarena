...
namespace ChobiAssets.PTM
{
	...
	public class Damage_Control_Center_CS : MonoBehaviour
	{
		...
		*/
        ...
        public void SetupTankInfo(TankSetting tankInfo)
        {
            print("InitTankInfo " + JsonConvert.SerializeObject(tankInfo));
            MainBody_HP = tankInfo.BodyHitPoint;
            Turret_Props[0].hitPoints = tankInfo.TurletHitPoint;
        }
        ...
        {
            string namePlayer = transform.parent.name;
            print("Destroy turlet " + namePlayer);
            GameManager.Instance.BattleManager.OnTankDestroy(namePlayer);
            if (Turret_Props[index].turretBaseTransform == null)
            { // The turret had already been destroyed.
                return;
            }
